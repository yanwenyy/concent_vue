    ...
    <div style="overflow: hidden;">
      <div class="window">
        ...
        <div class="imgBox">
          ...
          <el-button-group style="width:100%;margin-top:-2px;border-radios:0;display: flex;" v-if="!this.fileData">
            <el-button
          ...
          <el-button-group style="width:100%;margin-top:-2px;border-radios:0;display: flex;" v-if="this.fileData">
            ...
            <el-button
              @click.native="downloadimg()"
              size="samll"
              style="width:50%;border-radios:0;"
              type="primary"
              v-if="file.id"
            >图片下载</el-button>
            <el-button
      ...
      <div class="rightBox">
        <template>
          <el-checkbox-group @change="radioChg" class="checkpic" v-model="radio">
            ...
            <div class="ulBox">
              <span class="imgtitle">整体设计图</span>
              <div class="ulImgBox">
                <ul class="dots">
                    <template   v-for="(dot, i) in ztsjt">
                  <li
                    :class="{dotted:  dot.url === imgUrl}"
                    :key="i"
                    @click="jump(dot,ztsjfile[i])"
                    style="position:relative;"
            ...
            <div class="ulBox">
              <span class="imgtitle">细节设计图</span>
              <div class="ulImgBox ulImgBox1" ref="ulImgBox1">
                <el-button
                  @click="toleft1"
                  circle
                  class="toleftarrow"
                ...
                <ul class="dots">
 <template   v-for="(dot, i) in xjsjt">
                  <li
                    :class="{dotted:  dot.pic.url === imgUrl}"
                    :key="i"
                    @click="jump(dot.pic,dot.files[0])"
                    style="position:relative;"
v-if="dot.pic.id"
                  >
                    <el-checkbox
                      ...
                      :label="dot.pic.id"
                      style="position:absolute;width:20px;right:0;"
                      v-if="uuid!==''&&dot.pic.id"
                    ></el-checkbox>
                    <img v-if="dot.pic.id" :src="dot.pic.url" alt style="width:100%;height:100%;" />
                  </li>
 </template>
                </ul>
                <el-button
    ...
    <div class="imgcontent" v-html="content" v-if="content"></div>
  </div>
</template>
<script>
export default {
  ...
  created() {
    setTimeout(() => {
      if (this.yyt.length) {
        ...
        this.file = this.xjsjt[0].files
        this.imgUrl = this.xjsjt[0].pic.url
        this.content = this.xjsjt[0].pic.filesDescribe.descText
      }
    }, 1000)
  },
  methods: {
    ...
    toleft1() {
      this.$el.querySelector('.ulImgBox1').scrollLeft -= 100
    },
    toright1() {
      this.$el.querySelector('.ulImgBox1').scrollLeft += 100
    },
    ...
    downloadfile() {
      var path =
        this.$baseURL +
        '/api/contract/common/Files/downloadFile?fileId=' +
        this.fileData.id
      window.open(path)
    },
    jump(data, fileData) {
