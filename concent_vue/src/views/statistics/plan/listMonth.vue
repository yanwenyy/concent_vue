  ...
  export default {
    ...
    methods: {
      ...
      rowEdit(row) {
        ...
        let p = {actpoint: 'look', planInfo: {planId: row.uuid, projectName: row.projectName, planTypeName: '年计划', projectStatus: flowStatus, planProjectTjx: {projectId: row.projectId, planYear: row.planYear, planMonth: row.planMonth, planType: 1, projecttypeCode: row.projectTypeFirstId}}}
        this.$router.push({
          path: '../proTjxDetail/',
          query: { p: this.$utils.encrypt(JSON.stringify(p)) }
        })
