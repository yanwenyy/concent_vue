  ...
  <div>
    ...
    <div style="margin-top: 10px">
      ...
      <!--  <el-table-column
        ...
        >
          <template slot-scope="scope">
            <div>{{scope.row.flowStatus=='edit'?'草稿':scope.row.flowStatus=='check'?'审核中':scope.row.flowStatus=='pass'?'审核通过':scope.row.flowStatus=='reject'?'审核退回':'未创建'}}
            </div>
          </template>
         <!-- <template slot="header"
                    slot-scope="scope">
  ...
  export default {
    ...
    data() {
      return {
        ...
        yearDateS:JSON.parse(this.$utils.decrypt(this.$route.query.p)).reportYear+'-'+JSON.parse(this.$utils.decrypt(this.$route.query.p)).reportMonth,
        userdata:{},
        treeStatas: false,
