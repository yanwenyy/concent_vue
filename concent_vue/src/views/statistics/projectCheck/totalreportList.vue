...
<!--    <el-collapse value="projectInfo">
    ...
    <div style="margin-top: 10px">
      ...
      >
        ...
        >
          <template slot-scope="scope">
             {{scope.row.flowStatus=='notpass'?'草稿':scope.row.flowStatus=='edit'?'审核中':scope.row.flowStatus=='reject'?'审核通过':scope.row.flowStatus=='check'?'审核驳回':'待登记'}}
          </template>
          <template slot="header" slot-scope="scope">
            <span>状态</span>
            <div>
  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        ...
        // this.searchform.createOrgCode =this.p.orgCode
        // this.searchform.reportProjectName =this.p.projectName
        // this.searchform.reportYear = sj[0]
        // this.searchform.reportMonth = sj[1]
        // this.searchform.status='1'
        // this.searchform.flowStatus='1'
        // this.searchform.yearDateS=years
        this.$http
          .post('/api/statistics/inspectMonthReportFill/listMonthReport/queryMonthReportList', this.searchform)
          .then(res => {
