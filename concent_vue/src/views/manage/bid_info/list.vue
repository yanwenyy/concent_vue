    ...
    <div style="margin-top: 10px">
      ...
      >
        ...
        <el-table-column
          ...
          show-overflow-tooltip>
          <template slot-scope="scope">
              {{scope.row.flowStatus=='edit'?'草稿':scope.row.flowStatus=='check'?'审核中':scope.row.flowStatus=='pass'?'审核通过':scope.row.flowStatus=='reject'?'审核退回':scope.row.flowStatus==null?'待登记':'其他'}}
          </template>
          <template slot="header" slot-scope="scope">
            <span>审核状态</span>
            <div>
...
export default {
  ...
  methods: {
    ...
    rowshow(row) {
      ...
      let p = { actpoint: "look", instid: id , topInfoOrgId:row.topInfoOrgId,flowStatus:row.flowStatus,uuid:row.uuid};
      this.$router.push({
        path: "./detail/",
        query: { p: this.$utils.encrypt(JSON.stringify(p)) },
      });
