<template>
  <div style="position: relative">
    ...
    <el-button v-show="p.actpoint != 'look'&&p.actpoint != 'task'&&(p.actpoint == 'addk'||detailform.topInforBO.topInfor.bidFlowStatus=='edit'||detailform.topInforBO.topInfor.bidFlowStatus=='reject')" @click="saveInfo('detailform','sub')" class="detailbutton detail-back-tab sub-btn">提交</el-button>
    ...
    <el-tabs type="border-card" >
            ...
            <el-row>
              <p class="detail-title"><span style="font-size: 14px;color: #606266;" >开标记录（最大10MB）: </span>
                ...
                <el-upload
                  ...
                  :on-change="( file, fileList)=>{uploadPorgress( file, fileList,detailform.bidInfo_02)}"
      ...
      <el-tab-pane label="审批流程" v-if="p.actpoint == 'task'||p.actpoint == 'look'">
        <Audit-Process :task="(p.task||p.from=='kblist')&&p.instid.indexOf('-kb')==-1?{businessId:p.instid+'-kb',businessType:' contract_bid_register'}:{businessId:p.instid,businessType:' contract_bid_register'}"></Audit-Process>
      </el-tab-pane>
    </el-tabs>
