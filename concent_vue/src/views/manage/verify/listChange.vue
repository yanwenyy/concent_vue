  ...
  <div>
    <div style="width: 100%; overflow: hidden">
      <el-button-group style="float: left">
        ...
        <el-button :disabled="flowStatus=='edit' || flowStatus=='reject'" @click="editItem" plain type="primary"><i class="el-icon-edit"></i>修改</el-button>
        ...
        >
          <template slot-scope="scope">
            <!-- {{scope.row.uuid==null?'未通过':'通过'}} -->
             {{scope.row.flowStatus=='notpass'?'草稿':scope.row.flowStatus=='edit'?'审核中':scope.row.flowStatus=='reject'?'审核通过':scope.row.flowStatus=='check'?'审核退回':'其他情况'}}
          </template>
         <template slot="header" slot-scope="scope">
            <span>资格预审结果</span>
            <div>
