...
export default {
  ...
  methods: {
    ...
    setYearSale(year,index){
      var yearSale=0,yearSaleNext=0,totalMoney=0;
      this.detailform.contractInfoHouseSalesList.forEach((item,i)=>{
        ...
        totalMoney+=Number(item.monthSales);
    });
      this.detailform.contractInfoHouseSalesList.forEach((item,i)=>{
    ...
    handleRemove1(file,index) {
      ...
      console.log(this.detailform.commonFilesList1)
    },
    ...
    saveInfo(formName,type) {
      ...
      if(this.detailform.topInfoSiteList.length==0){
          this.$message.error("请至少选择一个合同生效地点");
          return false;
        }else{
          let k = 0;
          ...
          if(k===1){
            this.$message.error("请正确选择合同生效地点");
            return false;
          }
        }
      var hasMain=false;
